...
public sealed class ControlPageApiTests
{
    ...
    public async Task StartAsync_DefaultConfiguration_BindsRandomIpv4LoopbackAndServesEmbeddedAssets()
    {
        ...
        Assert.Contains("Five-segment side-light preview", html, StringComparison.Ordinal);
        ...
        Assert.Contains("Abandon old baseline ownership", html, StringComparison.Ordinal);
        Assert.Contains("Session diagnostics", html, StringComparison.Ordinal);
        Assert.Contains("memory only", html, StringComparison.Ordinal);
        Assert.Contains("takes effect after M4 installation", html, StringComparison.Ordinal);
        Assert.Contains("Device identity", html, StringComparison.Ordinal);
        Assert.Contains("Descriptor / interface fingerprint", html, StringComparison.Ordinal);
        Assert.Contains("id=\"interface-fingerprint\">Unknown", html, StringComparison.Ordinal);
        Assert.Contains("value=\"dongle\" disabled", html, StringComparison.Ordinal);
        Assert.Contains("dongle (diagnostic only)", html, StringComparison.Ordinal);
        ...
        Assert.Contains("device.interfaceFingerprint || \"Unknown\"", javaScript, StringComparison.Ordinal);
